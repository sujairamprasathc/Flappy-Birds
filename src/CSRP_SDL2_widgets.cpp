...
void AlertDialog::handleEvent(SDL_Event &e) {
  ...
  if (e.type == SDL_MOUSEBUTTONDOWN) {
    ...
    if (x > 140 and x < 180 and y > 50 and y < 70) close();
  }
}

bool AlertDialog::create(std::string WINDOW_TITLE) {
  if (SDL2Window::init(WINDOW_TITLE, 320, 80, SDL_WINDOWPOS_UNDEFINED,
                       SDL_WINDOWPOS_UNDEFINED) == true) {
...
void AlertDialog::setAlertDialog(std::string text) { alertDialog = text; }
...
FilePickerDialog::FilePickerDialog() { baseDirectory = ""; }
